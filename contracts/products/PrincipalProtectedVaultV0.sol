 ...
 */
contract PrincipalProtectedVaultV0 is Initializable, ERC20Upgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function collectReward() private returns(uint256 tokenReward) {
    ...
    if (_crv > 0) {
      ...
      ISwapper(dex).swap(crv, vaultToken, _crv);
    }
    uint256 _after = IERC20(vaultToken).balanceOf(address(this));
